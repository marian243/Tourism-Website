	...
	<div id="page">
		...
		<aside id="colorlib-hero">
			<div class="flexslider">
				<ul class="slides">
			   	<li style="background-image: url(images/discount.jpg);">
...
$db1 = mysqli_connect('localhost', $user, $pass, $db) or die("couldn't conect");
$sql = "SELECT  tour.tour_name,tour.cost,tour.Cost-(tour.Cost*.45)
from tour,discount

where tour.dis_code = discount.dis_code

AND discount.dis_code =1";
$sql1 ="SELECT  tour.tour_name,tour.cost,tour.Cost-(tour.Cost*.35)
from tour,discount

where tour.dis_code = discount.dis_code

AND discount.dis_code =2";
...
$result =mysqli_query($db1,$sql);  
    //echo "something";
          ...
          {
              echo '<div style="font-size:2em;color:Black">STUDENT DISCOUNT 45%: </div>';
              echo nl2br("\n");
              //$_SESSION['variable_name_1'] = $result->fetch_assoc();
              while($row = $result->fetch_assoc())
              {
                  echo "Place :". $row["tour_name"]."Actual Cost :". $row["cost"]." After Discount :". $row["tour.Cost-(tour.Cost*.45)"]."<br>";
...
$result1 =mysqli_query($db1,$sql1);  
 ...
 if($result1->num_rows>0)
          {
              echo '<div style="font-size:2em;color:Black">HONEYMOON DISCOUNT 35%: </div>';
              echo nl2br("\n");
              //$_SESSION['variable_name_1'] = $result->fetch_assoc();
              while($row = $result1->fetch_assoc())
              {
                  echo "Place :". $row["tour_name"]." Actual Cost:". $row["cost"]." After Discount :". $row["tour.Cost-(tour.Cost*.35)"]."<br>";
                  echo '<div class="animate-box text-center">';
                  echo '<p><a href="book.php" class="btn btn-primary">Book Now!</a></p>';
                  echo '</div>';;
              }
